...
void ChattingServer::ContentTPSThread(void)
{
	while (true)
	{
		...
		_playerDistributeMap.clear();
		for (int32 i = 0; i < 50; i++)
		{
			for (int32 j = 0; j < 50; j++)
			{
				_playerDistributeMap[_userList[i][j].size()]++;
			}
		}
...
void ChattingServer::SendDataToMonitorThread(void)
{
	...
	while (true)
	{
		...
		monitorClient.mp_MonitorData(pPacket, en_PACKET_SS_MONITOR_DATA_UPDATE, dfMONITOR_DATA_TYPE_CHAT_SERVER_MEM, g_Monitor.GetProcessUserMemoryUsage(), time(NULL));
		monitorClient.SendPacket(pPacket);
		...
		monitorClient.mp_MonitorData(pPacket, en_PACKET_SS_MONITOR_DATA_UPDATE, dfMONITOR_DATA_TYPE_CHAT_PLAYER, GetPlayerCount(), time(NULL));
		monitorClient.SendPacket(pPacket);
		...
		monitorClient.mp_MonitorData(pPacket, en_PACKET_SS_MONITOR_DATA_UPDATE, dfMONITOR_DATA_TYPE_CHAT_UPDATEMSG_POOL, GetJobPacketPoolSize(), time(NULL));
		monitorClient.SendPacket(pPacket);
		...
		monitorClient.mp_MonitorData(pPacket, en_PACKET_SS_MONITOR_DATA_UPDATE, dfMONITOR_DATA_TYPE_MONITOR_NETWORK_RECV, g_Monitor.GetEthernetRecvBytes(), time(NULL));
		monitorClient.SendPacket(pPacket);

		monitorClient.mp_MonitorData(pPacket, en_PACKET_SS_MONITOR_DATA_UPDATE, dfMONITOR_DATA_TYPE_MONITOR_NETWORK_SEND, g_Monitor.GetEthernetSendBytes(), time(NULL));
		monitorClient.SendPacket(pPacket);

		// SendPacket 내부 SendPost 활성화
		monitorClient.ActiveSendPacket();

		monitorClient.mp_MonitorData(pPacket, en_PACKET_SS_MONITOR_DATA_UPDATE, dfMONITOR_DATA_TYPE_MONITOR_AVAILABLE_MEMORY, g_Monitor.GetAvailableMemoryUsage() / 1024 / 1024, time(NULL));
		monitorClient.SendPacket(pPacket);
...
void ChattingServer::SendPacket_SectorAround(User* pUser, Packet* pPacket)
{
	...
	for (int32 cnt = 0; cnt < sectorCnt; cnt++)
	{
		...
		for (auto iter = pList.begin(); iter != pList.end(); iter++)
		{
			SendPacket((*iter)->sessionId, pPacket);
		}
	}
}
